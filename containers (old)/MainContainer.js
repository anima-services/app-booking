...
export default function MainContainer(props) {
  ...
  const [selectedObject,
    selectObject] = useState(0);
  const [objectVisible,
    setObjectVisible] = useState(false);

  const [clickCount,
    setClickCount] = useState(0);
  const [lastClickTime,
    setLastClickTime] = useState(0);
  const handlePress = () => {
    const now = Date.now();
    if (now - lastClickTime <= 500) {
      // Проверяем, что между нажатиями прошло не более 500 мс
  ...
  function checkBusy() {
    ...
    for (let i = 0; i < config.spaces.length; i++) {
      let _itemData = data[`space_${config.spaces[i]}`];
          ...
          return <Pressable
            ...
            <View style={[Styles.fill,
              ...
              <View style={[Styles.overlay,
                _itemData.is_reserved_now ? Styles.busy: Styles.free]} />
              <LinearGradient
