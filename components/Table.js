...
const Table = (props) => {
    ...
    useEffect(() => {
        try {
            ...
            _eventList = _eventList.filter(item => {
                if (!item.status
                    ...
                    || item.status === 'automatically_canceled') return false;
                return true;
            });
            ...
            for (let i = 0; i < _eventList.length; i++) {
                ...
                if (_busyState || (!_wasSelected && _nowDate < (new Date(item.start)))) {
                ...
                _tableEvents.push(
                    ...
                    >
                        <TableEvent
                            ...
                            busy={_busyState}
                            onPress={() => { _checkIn(); props.selectMenu(2); }}
                        />
                    </View>
                );
            }
        ...
        } catch (e) { console.log('Ошибка при отрисовке графика: ' + e); }
    }, [props.reservationsData, props.date]);

    function getTime(in_date) {
        try {
            in_date = new Date(in_date);
            return `${String(in_date.getHours()).padStart(2, '0')}:${String(in_date.getMinutes()).padStart(2, '0')}`;
        } catch (e) {
    ...
    return (
        items.length > 0 ?
            ...
            /* График событий */
            <ScrollView showsVerticalScrollIndicator={false} ref={scrollViewRef} style={Styles.scrollTable}>
                {items}
            </ScrollView> :

            /* Плейсхолдер с пустым графиком */
            <View style={Styles.tablePlaceHolder}>
                <FontAwesomeIcon style={Styles.tablePlaceHolderLogo} icon="fa-regular fa-calendar-check" />
                <Text style={[Styles.text_regular, { textAlign: 'center' }]}>На сегодня ничего не запланировано!</Text>
