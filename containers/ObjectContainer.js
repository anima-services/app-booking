...
export default function ObjectContainer(props) {
    ...
    const [userSelector, selectUsers] = useState({ open: false, singleSelection: false, users: [] });
    const [selected, select] = useState([]);

    const [reservationSelected, selectReservation] = useState(undefined);
    ...
    return (
        <View style={[Styles.main, busy ? Styles.main.busy : Styles.main.free]}>
            <Menu
                logs={props.logs} date={props.date}
